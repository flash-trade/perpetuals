...
pub fn fill_order(ctx: Context<FillOrder>, _params: &FillOrderParams) -> Result<()> {
    ...
    require_gte!(order_expiry_time, curtime, LimitOrderError::OrderExpired);

    if order_side == Side::Long {
        require_gte!(exit_price, limit_price, LimitOrderError::MaxPriceSlippage);
    } else {
        require_gte!(limit_price, exit_price, LimitOrderError::MaxPriceSlippage);
    }
    ...
    perpetuals::cpi::close_position(
        ...
        ClosePositionParams { price: limit_price },
    )?;
